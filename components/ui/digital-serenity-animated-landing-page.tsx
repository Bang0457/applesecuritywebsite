...
export interface DigitalSerenityProps {
  /** When true, fills the positioned parent (e.g. hero `absolute inset-0`) instead of full viewport height */
...
const DigitalSerenity: React.FC<DigitalSerenityProps> = ({
  ...
  const gridPatternId = `grid-react-dark-${patternUid}`;
  ...
  useEffect(() => {
    ...
    const handleMouseEnter = (e: Event) => {
      ...
      t.style.textShadow = "0 0 20px rgba(203, 213, 225, 0.5)";
    };
  ...
  return (
    <>
      ...
      <div
        className={cn(
          "bg-gradient-to-br from-slate-900 via-black to-slate-800 text-slate-100 font-primary overflow-hidden relative",
          fillContainer ? "absolute inset-0 min-h-full h-full w-full" : "min-h-screen",
          className,
        )}
      >
        <svg
        ...
        <div className="relative z-10 min-h-screen flex flex-col justify-between items-center px-6 py-10 sm:px-8 sm:py-12 md:px-16 md:py-20 pointer-events-none">
        ...
        <div
          className="digital-serenity-mouse-gradient w-60 h-60 blur-xl sm:w-80 sm:h-80 sm:blur-2xl md:w-96 md:h-96 md:blur-3xl"
          style={{
